...
var L01Poem;
(function (L01Poem) {
    let subjekt = ["Hermine", "Harry", "Ron", "Hagrid", "Dobby", "Der sprechende Hut"];
    let praedikat = ["stiehlt", "verzaubert", "isst", "verflucht", "liebt", "belehrt"];
    let objekt = ["Schlafbohnen", "Drachen", "Dumbledores Phönix", "den Schnatz", "Krummbein", "die Heiligtümer des Todes"];
    ...
    function getRandomNum(_nameOfArray) {
        let getNum = Math.floor(Math.random() * _nameOfArray.length);
        //console.log(getNum);
        return getNum;
    }
    function getVerse() {
        //Aus jedem Array wird eine Zufallszahl gewählt, welche dann den Index repräsentiert
        let iSubjekt = getRandomNum(subjekt);
        let iPraedikat = getRandomNum(praedikat);
        let iObjekt = getRandomNum(objekt);
