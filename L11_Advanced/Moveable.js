...
(function (L11_1_Blumenwiese) {
    class Moveable {
        ...
        move(_timeSlice) {
            if (this.type == "Bee") {
                this.velocity = new L11_1_Blumenwiese.Vector(1100, 650);
                this.velocity.random(L11_1_Blumenwiese.createRandomNum(-300, 300), L11_1_Blumenwiese.createRandomNum(-600, 600));
            }
            if (this.type == "Cloud") {
                let offset = new L11_1_Blumenwiese.Vector(this.velocity.x, 0);
            ...
            if (this.position.x < 0)
                this.position.x += L11_1_Blumenwiese.crc2d.canvas.width;
            if (this.position.y < 0)
                this.position.y += L11_1_Blumenwiese.crc2d.canvas.height;
            if (this.position.x > L11_1_Blumenwiese.crc2d.canvas.width)
                this.position.x -= L11_1_Blumenwiese.crc2d.canvas.width;
